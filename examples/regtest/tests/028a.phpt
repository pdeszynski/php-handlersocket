--TEST--
HandlerSocketIndex: insert/update/delete
--SKIPIF--
--FILE--
<?php
...
{
    $hs = new HandlerSocket(MYSQL_HOST, MYSQL_HANDLERSOCKET_PORT_WR);
    $index = $hs->createIndex(2, MYSQL_DBNAME, $table, '', 'v1');
}
catch (HandlerSocketException $exception)
{
    echo $exception->getMessage(), PHP_EOL;
...
for ($i = 0; $i < $tablesize; $i++)
{
    ...
    {
        $retval = $index->update(
            $k, array(HandlerSocket::UPDATE => array('mod_' . $i)), 1000, 0);
    }
    else
    {
        $retval = $index->update(
            $k, array(HANDLERSOCKET_UPDATE => array('mod_' . $i)), 1000, 0);
    }
    if ($retval === false)
    {
        echo $index->getError(), PHP_EOL;
...
{
    $hs = new HandlerSocket(MYSQL_HOST, MYSQL_HANDLERSOCKET_PORT_WR);
    $index = $hs->createIndex(3, MYSQL_DBNAME, $table, '', '');
}
catch (HandlerSocketException $exception)
{
    echo $exception->getMessage(), PHP_EOL;
...
for ($i = 0; $i < $tablesize; $i = $i + 2)
{
    ...
    {
        $retval = $index->update(
            array($k), array(HandlerSocket::DELETE => null), 1000, 0);
    }
    else
    {
        $retval = $index->update(
            array($k), array(HANDLERSOCKET_DELETE => null), 1000, 0);
    }
    if ($retval === false)
    {
        echo $index->getError(), PHP_EOL;
