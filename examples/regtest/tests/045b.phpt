--TEST--
HandlerSocketIndex: IN, filters and modifications (update method)
--SKIPIF--
--FILE--
<?php
...
try
{
    ...
    $index1 = $hs->createIndex(1, MYSQL_DBNAME, $table, '', 'k,v,v2',
                             array('filter' => 'v2'));
...
$retval = $index2->update(
    array('=' => array('')), array('U' => array('MOD')), 10000, 0,
    array('filter' => array('=', 'v2', '1'),
          'in' => array(array('k10', 'k20x', 'k30', 'k40', 'k50'))));
if (!$retval)
{
...
$retval = $index1->find(array('>=' => array('')), 10000, 0);
if (!$retval)
{
